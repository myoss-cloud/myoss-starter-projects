...
/**
 * 测试开启自动记录 controller 异常，使用 {@link EnableAopLogController} 开启此功能
 *
 * @author Jerry.Chen
...
public class AopLogControllerExceptionHandlerCase1Tests {
    ...
    public void errorPostTest1() throws Exception {
        User user = getUser();
        String content = JSON.toJSONString(user) + "123456789";
    ...
    public void handleExceptionTest3() {
        ...
        Assertions.assertThat(JSON.toJSONString(parameterMap)).isEqualTo("{\"key\":[\"value\"],\"name\":[\"Jerry\"]}");
        BizRuntimeException bizRuntimeException = new BizRuntimeException("mock exception");
        ResponseEntity<Object> responseEntity = handler.handleException(bizRuntimeException, request);
